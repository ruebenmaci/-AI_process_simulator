...
std::string getEOSForTray(
   ...
   if (eosMode == "manual" && !eosManual.empty()) return eosManual;

   const double a = double(trayIndex) / std::max(1, trays - 1);
   if (a > 0.66) return "SRK";
   if (a > 0.33) return "PR";
   return "PRSV";
}
...
)
{
   ...
   // Murphree efficiency on K
   // React parity: if eta is missing/NaN, default it (otherwise Kmin can collapse toward ~0)
   bool useEta = finite(murphreeEtaV);
   double etaK = useEta ? murphreeEtaV : 0.55;

   if (!useEta) {
      // Log once per tray to prove this is happening at runtime
      int& ec = g_etaDefaultCounts[trayIndex];
      if (ec < 1) {
         ec++;
         emitLog("[EOSK_ETA_DEFAULT] tray=" + std::to_string(trayIndex + 1) +
            " eta_in=NaN -> eta=0.55");
      }
      useEta = true;
   }
   etaK = clamp(etaK, 0.0, 1.0);
   auto applyEta = [&](double kEq) {
      if (!useEta)
         return kEq;
      return 1.0 + etaK * (kEq - 1.0);
   };
   ...
   for (int it = 0; it < maxIter; it++) {
      ...
      for (int i = 0; i < n; i++) {
         ...
         const double kv = clampK(applyEta(kvEq));
