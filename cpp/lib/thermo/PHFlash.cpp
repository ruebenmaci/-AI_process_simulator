    ...
    if (eosSaysSinglePhase) {
      ...
      if (!in.disableSinglePhaseShortCircuit) {
         ...
         if (in.log) {
            ...
            logRRSignTest(in, T, "Kraw", ek.K, kmin_ss, kmax_ss, /*eosSinglePhase=*/true);
            logRRSignTest(in, T, "Kuse", ek.K, kmin_ss, kmax_ss, /*eosSinglePhase=*/true);
         }
...
EqTPResult equilibriumEnthalpy_TP(
   double P,
   double T,
   const std::vector<double>& z,
