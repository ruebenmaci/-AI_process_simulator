...
#include "../lib/sim/CounterCurrentColumnSimulator.hpp"
#include "../lib/thermo/pseudocomponents/loaders.hpp"
...
SolverOutputs solveColumn(
  ...
  CrudeSet cs = getCrudeSet(in.crudeName);

  SimulationOptions opt;
  opt.crudeName = in.crudeName;
  opt.trays = std::max(2, in.trays);
  opt.feedRate_kgph = std::max(0.0, in.feedRateKgph);

  // Components + z (keep pointers into CrudeSet so we don't copy large arrays)
  opt.components = &cs.components;
  opt.kij = &cs.kij;

  const size_t NC = cs.components.size();
  if (cs.hasZDefault && cs.zDefault.size() == NC) {
    opt.feedZ = cs.zDefault;
  } else {
