...
bool ComponentManager::saveUserListsToJsonFile(const QString& path) const
{
   // Save only user-created lists — those whose source is not "starter".
   // This keeps the save file small and avoids duplicating seed data that
   // is always re-created from the embedded resource on startup.
   QJsonArray listArr;
   for (const auto& list : componentLists_) {
      if (list.source.compare(QStringLiteral("starter"), Qt::CaseInsensitive) == 0)
         continue;
      listArr.append(list.toJson());
   }

   QJsonObject root;
   root.insert(QStringLiteral("schemaVersion"), QStringLiteral("1.0"));
   root.insert(QStringLiteral("fileType"), QStringLiteral("userComponentLists"));
   root.insert(QStringLiteral("componentLists"), listArr);
...
bool ComponentManager::mergeUserListsFromJsonFile(const QString& path)
{
   QFile file(path);
   if (!file.open(QIODevice::ReadOnly)) {
      emit errorOccurred(QStringLiteral("Could not open user lists file: %1").arg(path));
      return false;
   }
   ...
   if (parseError.error != QJsonParseError::NoError || !doc.isObject()) {
      emit errorOccurred(QStringLiteral("User lists JSON parse error in %1: %2")
         .arg(path, parseError.errorString()));
      return false;
   }

   const QJsonArray listArr = doc.object().value(QStringLiteral("componentLists")).toArray();
   bool anyChanged = false;
   for (const auto& v : listArr) {
      ...
      if (rec.id.trimmed().isEmpty()) continue;

      // Mark as user-created so it is included in future saveUserListsToJsonFile calls.
      if (rec.source.trimmed().isEmpty())
         rec.source = QStringLiteral("user");

      const int idx = indexOfListId_(rec.id);
      if (idx >= 0) {
         // Replace existing list (user may have edited it since last save).
         componentLists_[static_cast<std::size_t>(idx)] = std::move(rec);
      }
      else {
...
bool ComponentManager::createComponentList(const QString& name)
{
   ...
   rec.source = QStringLiteral("user");
   if (rec.listType.trimmed().isEmpty()) rec.listType = QStringLiteral("user-defined");
   componentLists_.push_back(std::move(rec));
   syncGroupModel_();
   emit componentListsChanged();
   return true;
...
QVariantMap ComponentManager::describeComponentList(const QString& listId) const
{
   return componentListSummary(listId);
}

QVariantMap ComponentManager::validateComponentList(const QString& listId) const
{
   const int idx = indexOfListId_(listId);
   if (idx < 0) {
      QVariantMap out;
      out.insert(QStringLiteral("id"), listId);
      out.insert(QStringLiteral("valid"), false);
      out.insert(QStringLiteral("statusText"), QStringLiteral("Component list was not found."));
      out.insert(QStringLiteral("missingComponentCount"), 0);
      out.insert(QStringLiteral("resolvedComponentCount"), 0);
      out.insert(QStringLiteral("declaredComponentCount"), 0);
      out.insert(QStringLiteral("listType"), QString());
      out.insert(QStringLiteral("sourceFluidName"), QString());
      out.insert(QStringLiteral("missingComponentIds"), QVariantList{});
      return out;
   }
   return buildComponentListValidation_(componentLists_[static_cast<std::size_t>(idx)]);
}

QVariantMap ComponentManager::componentListSummary(const QString& listId) const
{
   const int idx = indexOfListId_(listId);
   if (idx < 0) return {};

   const auto& rec = componentLists_[static_cast<std::size_t>(idx)];
   QVariantMap out = rec.toVariantMap();
   const QVariantMap validation = buildComponentListValidation_(rec);
   for (auto it = validation.constBegin(); it != validation.constEnd(); ++it)
      out.insert(it.key(), it.value());
   out.insert(QStringLiteral("componentCount"), rec.componentIds.size());
   out.insert(QStringLiteral("displayName"), rec.name);
   return out;
}

FluidDefinition ComponentManager::buildFluidDefinitionSkeletonFromComponentList(const QString& listId) const
{
   FluidDefinition fluid;

   const int idx = indexOfListId_(listId);
   if (idx < 0) return fluid;

   const auto& rec = componentLists_[static_cast<std::size_t>(idx)];
   fluid.name = rec.name.toStdString();

   std::vector<Component> comps;
   comps.reserve(static_cast<std::size_t>(rec.componentIds.size()));
   for (const auto& cid : rec.componentIds) {
      const int cidx = indexOfComponentId_(cid);
      if (cidx < 0) continue;
      const auto& src = components_[static_cast<std::size_t>(cidx)];
      Component c;
      c.name = src.name.toStdString();
      c.Tb = src.normalBoilingPointK.value_or(0.0);
      c.MW = src.molarMass.value_or(0.0);
      c.Tc = src.criticalTemperatureK.value_or(0.0);
      c.Pc = src.criticalPressurePa.value_or(0.0);
      c.omega = src.acentricFactor.value_or(0.0);
      c.SG = src.specificGravity60F.value_or(0.0);
      c.delta = src.volumeShiftDelta.value_or(0.0);
      comps.push_back(c);
   }

   fluid.thermo.components = std::move(comps);

   const std::size_t n = fluid.thermo.components.size();
   fluid.thermo.kij.assign(n, std::vector<double>(n, 0.0));
   if (n > 0) {
      fluid.thermo.hasZDefault = true;
      fluid.thermo.zDefault.assign(n, 1.0 / static_cast<double>(n));
   }
   return fluid;
}


QVariantMap ComponentManager::buildComponentListValidation_(const sim::ComponentListRecord& rec) const
{
   QVariantMap out;
   QVariantList missingIds;
   int resolvedCount = 0;
   ...
   for (const auto& cid : rec.componentIds) {
      const int cidx = indexOfComponentId_(cid);
      if (cidx < 0) {
         missingIds.push_back(cid);
         continue;
      }
      ++resolvedCount;
      const auto& comp = components_[static_cast<std::size_t>(cidx)];
      if (comp.componentType == sim::ComponentType::PseudoComponent) hasPseudo = true;
      else if (comp.componentType == sim::ComponentType::Pure) hasPure = true;
      else hasOther = true;
   }

   const int declaredCount = rec.componentIds.size();
   const int missingCount = missingIds.size();
   const bool valid = declaredCount > 0 && missingCount == 0;

   QString statusText;
   if (declaredCount == 0) statusText = QStringLiteral("Component list is empty.");
   else if (missingCount > 0) statusText = QStringLiteral("Component list references %1 missing component(s).").arg(missingCount);
   else statusText = QStringLiteral("Component list is valid and resolves %1 component(s).").arg(resolvedCount);

   QString inferredType = inferredListType_(rec);
   if (inferredType.isEmpty()) {
      if (hasPseudo && !hasPure && !hasOther) inferredType = rec.sourceFluidName.trimmed().isEmpty() ? QStringLiteral("pseudo-component") : QStringLiteral("pseudo-crude");
      else if (hasPure && !hasPseudo && !hasOther) inferredType = QStringLiteral("pure-component");
      else if (resolvedCount > 0) inferredType = QStringLiteral("mixed");
   }

   out.insert(QStringLiteral("id"), rec.id);
   out.insert(QStringLiteral("name"), rec.name);
   out.insert(QStringLiteral("valid"), valid);
   out.insert(QStringLiteral("statusText"), statusText);
   out.insert(QStringLiteral("declaredComponentCount"), declaredCount);
   out.insert(QStringLiteral("resolvedComponentCount"), resolvedCount);
   out.insert(QStringLiteral("missingComponentCount"), missingCount);
   out.insert(QStringLiteral("missingComponentIds"), missingIds);
   out.insert(QStringLiteral("listType"), inferredType);
   out.insert(QStringLiteral("sourceFluidName"), rec.sourceFluidName);
   out.insert(QStringLiteral("containsPseudoComponents"), hasPseudo);
   out.insert(QStringLiteral("containsPureComponents"), hasPure);
   out.insert(QStringLiteral("containsOtherComponentTypes"), hasOther);
   return out;
}

QString ComponentManager::inferredListType_(const sim::ComponentListRecord& rec) const
{
   const QString explicitType = rec.listType.trimmed();
   if (!explicitType.isEmpty()) return explicitType;
   if (!rec.sourceFluidName.trimmed().isEmpty()) return QStringLiteral("pseudo-crude");
   return QString();
}
...
void ComponentManager::ensureStarterComponentLists_(bool replaceStarterLists)
{
   ...
   for (const auto& fluidStd : listFluidDefinitions()) {
      ...
      rec.listType = QStringLiteral("pseudo-crude");
      rec.sourceFluidName = fluidName;
