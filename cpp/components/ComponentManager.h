...
public:
   ...
   // Saves only user-created component lists (source != "starter") to a
   // compact JSON file. Does not save components or binary interactions.
   // This is the preferred auto-save target for user list persistence.
   Q_INVOKABLE bool saveUserListsToJsonFile(const QString& path) const;

   // Merges user lists from a JSON file produced by saveUserListsToJsonFile
   // into the current state. Existing starter lists are preserved. Any list
   // whose id already exists is replaced with the saved version.
   Q_INVOKABLE bool mergeUserListsFromJsonFile(const QString& path);
   ...
   Q_INVOKABLE QVariantMap  describeComponentList(const QString& listId) const;
   Q_INVOKABLE QVariantMap  validateComponentList(const QString& listId) const;
   Q_INVOKABLE QVariantMap  componentListSummary(const QString& listId) const;
   FluidDefinition buildFluidDefinitionSkeletonFromComponentList(const QString& listId) const;
...
private:
   ...
   QVariantMap buildComponentListValidation_(const sim::ComponentListRecord& rec) const;
   QString inferredListType_(const sim::ComponentListRecord& rec) const;
