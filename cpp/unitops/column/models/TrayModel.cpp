...
QVariant TrayModel::data(const QModelIndex& index, int role) const {
   ...
   case XLiqRole: {
      QVariantList out;
      for (double v : r.xLiq) out.append(v);
      return out;
   }
   case YVapRole: {
      QVariantList out;
      for (double v : r.yVap) out.append(v);
      return out;
   }
   default:               return {};
   }
...
void TrayModel::setComponentNames(const QStringList& names)
{
   if (componentNames_ == names) return;
   componentNames_ = names;
