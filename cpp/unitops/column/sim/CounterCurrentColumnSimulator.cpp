...
SimulationResult simulateColumn(const SimulationOptions& opt) {
   ...
   for (int iter = 0; iter < opt.maxIter; ++iter) {
      ...
      const double L_ref_preRelax = L_ref;
      const double V_boil_preRelax = V_boil;
      ...
      const bool splitOK = (std::abs(Lref_new - L_ref_preRelax) < tolSplit) && (std::abs(Vboil_new - V_boil_preRelax) < tolSplit);
