...
Rectangle {
    ...
    function fmt0(x)  { const n = Number(x); return isFinite(n) ? Math.round(n).toString() : "—" }
    function fmtMs(ms) {
    ...
    function attachedStrippersSorted() {
        if (!appState || !appState.attachedStrippers)
            return []
        var rows = []
    ...
    function attachedStripperForSpec(spec) {
        if (!spec || !appState || !appState.attachedStrippers)
            return null
        var matches = attachedStrippersSorted()
        var tray = Number(spec.tray !== undefined ? spec.tray : -1)
        var stripperId = spec.stripperId ? String(spec.stripperId) : ""
        var label = spec.stripperLabel ? String(spec.stripperLabel) : ((spec.name ? String(spec.name) : "") + " Stripper")
        ...
        for (var j = 0; j < matches.length; ++j) {
            var row2 = matches[j]
            if (Number(row2.sourceTray) === tray)
                return row2
        }
        for (var k = 0; k < matches.length; ++k) {
            var row3 = matches[k]
            if (row3.label && String(row3.label) === label)
                return row3
        }
        return null
    }
    function stripperStatusColor(row) {
        if (!row)
            return textMuted
        if (row.hasErrors || String(row.status).toUpperCase() === "ERROR" || String(row.status).toUpperCase() === "FAIL")
            return errorRed
        if (row.hasWarnings || String(row.status).toUpperCase() === "WARN")
            return warnAmber
        ...
        Item {
            ...
            Item {
                ...
                Item {
                    ...
                    CompactFrame {
                        id: convCard2
                        x: 0; y: effCard2.y + effCard2.height + 4; width: parent.halfW
                        height: convHdr.height + convCol2.implicitHeight + 1

                        SectionHeader { id: convHdr; width: parent.width; text: "Solver Convergence" }
                        Column {
                            id: convCol2
                            anchors { left: parent.left; right: parent.right; top: convHdr.bottom }
                            Item { width: parent.width; height: rowH
                                Text { x: 6; anchors.verticalCenter: parent.verticalCenter; width: 160; text: "Max Outer Iterations"; font.pixelSize: fsLbl; color: textMuted }
                                CSpin { anchors { right: parent.right; rightMargin: 6; verticalCenter: parent.verticalCenter }
                                    from: 1; to: 1000
                                    value: appState ? appState.maxOuterIterations : 100
                                    onValueModified: { if (appState) appState.maxOuterIterations = value } }
                                HDivider { anchors.bottom: parent.bottom; width: parent.width }
                            }
                            Item { width: parent.width; height: rowH
                                Text { x: 6; anchors.verticalCenter: parent.verticalCenter; width: 160; text: "Outer Conv Tolerance"; font.pixelSize: fsLbl; color: textMuted }
                                CField { anchors { right: parent.right; rightMargin: 6; verticalCenter: parent.verticalCenter }
                                    width: 110
                                    text: appState ? Number(appState.outerConvergenceTolerance).toExponential(3) : "1.000e-4"
                                    onEditingFinished: { if (appState) appState.outerConvergenceTolerance = Number(text) } }
                            }
                        }
                    }
                } // Setup sub-tab
                ...
                Item {
                    ...
                    property real drawW:  (width - 6) * 0.74
                    property real solveW: (width - 6) - drawW
                    ...
                    CompactFrame {
                        ...
                        property int selectedDrawIndex: (appState && appState.drawSpecs && appState.drawSpecs.length > 0) ? Math.min(selectedDrawIndex, appState.drawSpecs.length - 1) : -1
                        ...
                        ListView {
                            id: drawSpecsList
                            anchors{left:parent.left;right:parent.right;top:drawColHdr.bottom;bottom:stripperPanel.visible ? stripperPanel.top : drawFooter2.top}
                            clip:true
                            ...
                            delegate: Item {
                                ...
                                Rectangle{anchors.fill:parent;color: drawCard2.selectedDrawIndex === rowIdx ? "#dbe8f6" : (index%2===0?rowEven:rowOdd)}
                                MouseArea{anchors.fill: parent; onClicked: { drawCard2.activateRow(rowIdx) } z:0}
                                Row {
                                    ...
                                    CField{
                                        ...
                                        text:spec.name||""
                                        onActiveFocusChanged: if (activeFocus) drawCard2.activateRow(rowIdx)
                                    ...
                                    CSpin{width:60;anchors.verticalCenter:parent.verticalCenter;from:2;to:appState?Math.max(2,appState.trays-1):30;value:spec.tray||1
                                        onValueModified:{drawCard2.activateRow(rowIdx); var s=appState.drawSpecs;var c=[];for(var k=0;k<s.length;k++)c.push(Object.assign({},s[k]));c[rowIdx].tray=value;drawCard2.commitSpecs(c)}}
                                    CCombo{width:52;anchors.verticalCenter:parent.verticalCenter;model:["L","V"];currentIndex:(spec.phase==="V")?1:0
                                        onActivated:{drawCard2.activateRow(rowIdx); var s=appState.drawSpecs;var c=[];for(var k=0;k<s.length;k++)c.push(Object.assign({},s[k]));c[rowIdx].phase=model[index];drawCard2.commitSpecs(c)}}
                                    CCombo{width:78;anchors.verticalCenter:parent.verticalCenter;model:["feedPct","kg/h"];currentIndex:(spec.basis==="kg/h")?1:0
                                        onActivated:{drawCard2.activateRow(rowIdx); var s=appState.drawSpecs;var c=[];for(var k=0;k<s.length;k++)c.push(Object.assign({},s[k]));c[rowIdx].basis=model[index];drawCard2.commitSpecs(c)}}
                                    CField{width:68;anchors.verticalCenter:parent.verticalCenter
                                        text:spec.value!==undefined?fmt2(Number(spec.value)):"0.00"
                                        onActiveFocusChanged: if (activeFocus) drawCard2.selectedDrawIndex = rowIdx
                                    ...
                                    Rectangle{
                                        width:56; height:rowH-4
                                        anchors.verticalCenter:parent.verticalCenter
                                        radius:3
                                        color: Boolean(spec.stripperEnabled) ? "#edf5ea" : "transparent"
                                        border.color: Boolean(spec.stripperEnabled) ? "#7aa46d" : borderIn
                                        border.width:1
                                        Text{anchors.centerIn:parent;text:Boolean(spec.stripperEnabled)?"Stripper On":"Stripper";font.pixelSize:9;color:Boolean(spec.stripperEnabled)?"#1a7a3c":textMuted;font.bold:true}
                                        MouseArea{
                                            anchors.fill:parent
                                            hoverEnabled:true; cursorShape:Qt.PointingHandCursor
                                            onClicked:{
                                                drawCard2.selectedDrawIndex = rowIdx
                                                ...
                                                if (!c[rowIdx].stripperLabel || c[rowIdx].stripperLabel === "") c[rowIdx].stripperLabel = (c[rowIdx].name || "Draw") + " Stripper"
                                                if (!c[rowIdx].stripperTrays) c[rowIdx].stripperTrays = 4
                                                if (!c[rowIdx].stripperReturnTray) c[rowIdx].stripperReturnTray = Math.max(2, Number(c[rowIdx].tray || 2) - 1)
                                                if (!c[rowIdx].stripperHeatMode || c[rowIdx].stripperHeatMode === "") c[rowIdx].stripperHeatMode = "Steam"
                                                if (c[rowIdx].stripperHeatValue === undefined) c[rowIdx].stripperHeatValue = 0
                                                drawCard2.activeStripperIndex = enableNext ? rowIdx : -1
                                                drawCard2.commitSpecs(c)
                                            }
                                        }
                                    }
                                    Rectangle{
                                        ...
                                        MouseArea{
                                            ...
                                            onClicked:{
                                                ...
                                                for(var k=0;k<s.length;k++) c.push(Object.assign({},s[k]))
                                                c.splice(rowIdx,1);
                                                if (drawCard2.selectedDrawIndex >= c.length) drawCard2.selectedDrawIndex = c.length - 1
                                                if (drawCard2.activeStripperIndex === rowIdx) drawCard2.activeStripperIndex = -1
                                                else if (drawCard2.activeStripperIndex > rowIdx) drawCard2.activeStripperIndex = drawCard2.activeStripperIndex - 1
                                                drawCard2.commitSpecs(c)
                                            }
                                        }
                                    }
                        ...
                        CompactFrame {
                            ...
                            function commitField(key, value) {
                                if (!appState || drawCard2.activeStripperIndex < 0) return
                                var s = appState.drawSpecs; var c = []
                                for (var k = 0; k < s.length; ++k) c.push(Object.assign({}, s[k]))
                                c[drawCard2.activeStripperIndex][key] = value
                                drawCard2.commitSpecs(c)
                            }

                            SectionHeader { id: stripperHdr; width: parent.width; text: "Attached Side Stripper" }

                            Column {
                                id: stripperConfigColumn
                                anchors { left: parent.left; right: parent.right; top: stripperHdr.bottom; leftMargin: 6; rightMargin: 6; topMargin: 14 }
                                spacing: 6

                                Row { spacing: 8; height: rowH
                                    Text { width: 60; text: "Label"; font.pixelSize: fsLbl; color: textMuted; verticalAlignment: Text.AlignVCenter }
                                    CField { width: 300; horizontalAlignment: Text.AlignLeft;
                                        text: stripperPanel.selectedSpec && stripperPanel.selectedSpec.stripperLabel ? stripperPanel.selectedSpec.stripperLabel : ((stripperPanel.selectedSpec && stripperPanel.selectedSpec.name ? stripperPanel.selectedSpec.name : "Draw") + " Stripper")
                                        onEditingFinished: stripperPanel.commitField("stripperLabel", text) }
                                }

                                Row { spacing: 8; height: rowH
                                    Text { width: 60; text: "Trays"; font.pixelSize: fsLbl; color: textMuted; verticalAlignment: Text.AlignVCenter }
                                    CSpin { width: 60; from: 2; to: 20; value: stripperPanel.selectedSpec && stripperPanel.selectedSpec.stripperTrays ? Number(stripperPanel.selectedSpec.stripperTrays) : 4
                                        onValueModified: stripperPanel.commitField("stripperTrays", value) }
                                }

                                Row { spacing: 8; height: rowH
                                    Text { width: 60; text: "Return Tray"; font.pixelSize: fsLbl; color: textMuted; verticalAlignment: Text.AlignVCenter }
                                    CSpin { width: 60; from: 2; to: appState ? Math.max(2, appState.trays - 1) : 30; value: stripperPanel.selectedSpec && stripperPanel.selectedSpec.stripperReturnTray ? Number(stripperPanel.selectedSpec.stripperReturnTray) : Math.max(2, Number(stripperPanel.selectedSpec && stripperPanel.selectedSpec.tray ? stripperPanel.selectedSpec.tray : 2) - 1)
                                        onValueModified: stripperPanel.commitField("stripperReturnTray", value) }
                                }

                                Row { spacing: 8; height: rowH
                                    Text { width: 60; text: "Heat Mode"; font.pixelSize: fsLbl; color: textMuted; verticalAlignment: Text.AlignVCenter }
                                    CCombo { width: 100; model: ["Steam", "ReboilerDuty"];
                                        currentIndex: (stripperPanel.selectedSpec && stripperPanel.selectedSpec.stripperHeatMode === "ReboilerDuty") ? 1 : 0
                                        onActivated: stripperPanel.commitField("stripperHeatMode", model[index]) }
                                    Text { width: 78; text: "Heat Value"; font.pixelSize: fsLbl; color: textMuted; verticalAlignment: Text.AlignVCenter }
                                    CField { width: 68;
                                        text: stripperPanel.selectedSpec && stripperPanel.selectedSpec.stripperHeatValue !== undefined ? fmt2(Number(stripperPanel.selectedSpec.stripperHeatValue)) : "0.00"
                                        onEditingFinished: stripperPanel.commitField("stripperHeatValue", Number(text)) }
                                    Text { text: (stripperPanel.selectedSpec && stripperPanel.selectedSpec.stripperHeatMode === "ReboilerDuty") ? "kW" : "kg/h steam"; font.pixelSize: fsSm; color: textMuted; verticalAlignment: Text.AlignVCenter }
                                }

                                Row { spacing: 8; height: rowH
                                    Text { width: 100; text: "Max Coupled Iter"; font.pixelSize: fsLbl; color: textMuted; verticalAlignment: Text.AlignVCenter }
                                    CSpin { width: 60; from: 1; to: 200; value: stripperPanel.selectedSpec && stripperPanel.selectedSpec.stripperMaxCoupledIterations ? Number(stripperPanel.selectedSpec.stripperMaxCoupledIterations) : 25
                                        onValueModified: stripperPanel.commitField("stripperMaxCoupledIterations", value) }
                                    Text { width: 70; text: "Coupling Tol"; font.pixelSize: fsLbl; color: textMuted; verticalAlignment: Text.AlignVCenter }
                                    CField { width: 76; text: stripperPanel.selectedSpec && stripperPanel.selectedSpec.stripperCouplingTolerance !== undefined ? Number(stripperPanel.selectedSpec.stripperCouplingTolerance).toExponential(3) : "1.000e-3"
                                        onEditingFinished: stripperPanel.commitField("stripperCouplingTolerance", Number(text)) }
                                }

                                Row { spacing: 8; height: rowH
                                    Text { width: 84; text: "Return Damping"; font.pixelSize: fsLbl; color: textMuted; verticalAlignment: Text.AlignVCenter }
                                    CField { width: 76; text: stripperPanel.selectedSpec && stripperPanel.selectedSpec.stripperReturnDamping !== undefined ? fmt3(Number(stripperPanel.selectedSpec.stripperReturnDamping)) : "0.350"
                                        onEditingFinished: stripperPanel.commitField("stripperReturnDamping", Number(text)) }
                                    Text { text: "0.0 - 1.0"; font.pixelSize: fsSm; color: textMuted; verticalAlignment: Text.AlignVCenter }
                                }
                            }

                            Rectangle {
                                visible: !!stripperPanel.selectedResult
                                anchors { left: parent.left; right: parent.right; top: stripperConfigColumn.bottom; leftMargin: 6; rightMargin: 6; topMargin: 14; bottom: parent.bottom; bottomMargin: 6 }
                                color: rowEven
                                border.color: borderIn
                                border.width: 1

                                SectionHeader { id: stripperResultHdr; width: parent.width; text: "Stripper Run Results" }

                                Column {
                                    anchors { left: parent.left; right: parent.right; top: stripperResultHdr.bottom; leftMargin: 6; rightMargin: 6; topMargin: 14 }
                                    spacing: 6

                                    Row {
                                        width: parent.width; height: rowH
                                        Text { width: 110; text: "Status"; font.pixelSize: fsLbl; color: textMuted; verticalAlignment: Text.AlignVCenter }
                                        Text { width: 90; text: stripperPanel.selectedResult ? String(stripperPanel.selectedResult.status || "—") : "—"; font.pixelSize: fsVal; font.bold: true; color: stripperStatusColor(stripperPanel.selectedResult); verticalAlignment: Text.AlignVCenter }
                                        Text { width: 80; text: "Feed"; font.pixelSize: fsLbl; color: textMuted; verticalAlignment: Text.AlignVCenter }
                                        Text { width: 90; text: stripperPanel.selectedResult ? (fmt2(Number(stripperPanel.selectedResult.feedKgph)) + " kg/h") : "—"; font.pixelSize: fsVal; color: valueBlue; verticalAlignment: Text.AlignVCenter }
                                        Text { width: 92; text: "Bottoms"; font.pixelSize: fsLbl; color: textMuted; verticalAlignment: Text.AlignVCenter }
                                        Text { text: stripperPanel.selectedResult ? (fmt2(Number(stripperPanel.selectedResult.bottomsProductKgph)) + " kg/h") : "—"; font.pixelSize: fsVal; color: valueBlue; verticalAlignment: Text.AlignVCenter }
                                    }
                                    Row {
                                        width: parent.width; height: rowH
                                        Text { width: 110; text: "Vapor Return"; font.pixelSize: fsLbl; color: textMuted; verticalAlignment: Text.AlignVCenter }
                                        Text { width: 90; text: stripperPanel.selectedResult ? (fmt2(Number(stripperPanel.selectedResult.vaporReturnKgph)) + " kg/h") : "—"; font.pixelSize: fsVal; color: valueBlue; verticalAlignment: Text.AlignVCenter }
                                        Text { width: 80; text: "Top T"; font.pixelSize: fsLbl; color: textMuted; verticalAlignment: Text.AlignVCenter }
                                        Text { width: 90; text: stripperPanel.selectedResult ? (fmt2(Number(stripperPanel.selectedResult.topTemperatureK)) + " K") : "—"; font.pixelSize: fsVal; color: valueBlue; verticalAlignment: Text.AlignVCenter }
                                        Text { width: 92; text: "Bottom T"; font.pixelSize: fsLbl; color: textMuted; verticalAlignment: Text.AlignVCenter }
                                        Text { text: stripperPanel.selectedResult ? (fmt2(Number(stripperPanel.selectedResult.bottomTemperatureK)) + " K") : "—"; font.pixelSize: fsVal; color: valueBlue; verticalAlignment: Text.AlignVCenter }
                                    }
                                    Row {
                                        width: parent.width; height: rowH
                                        Text { width: 110; text: "Source/Return"; font.pixelSize: fsLbl; color: textMuted; verticalAlignment: Text.AlignVCenter }
                                        Text { width: 90; text: stripperPanel.selectedResult ? (String(stripperPanel.selectedResult.sourceTray) + " / " + String(stripperPanel.selectedResult.returnTray)) : "—"; font.pixelSize: fsVal; color: valueBlue; verticalAlignment: Text.AlignVCenter }
                                        Text { width: 80; text: "Pressure"; font.pixelSize: fsLbl; color: textMuted; verticalAlignment: Text.AlignVCenter }
                        ...
                        Item {
                            ...
                            Row {
                                ...
                                ClassicButton{text:"+ Add Draw";width:86;baseColor:activeBlue
                                    onClicked:{if(!appState)return;var s=appState.drawSpecs;var c=[];for(var k=0;k<s.length;k++)c.push(Object.assign({},s[k]));c.push({name:"New Draw",tray:appState.feedTray||16,basis:"feedPct",phase:"L",value:0,stripperEnabled:false,stripperId:"",stripperLabel:"",stripperTrays:4,stripperReturnTray:Math.max(2,(appState.feedTray||16)-1),stripperHeatMode:"Steam",stripperHeatValue:0,stripperMaxCoupledIterations:25,stripperCouplingTolerance:1e-3,stripperReturnDamping:0.35});drawCard2.selectedDrawIndex=c.length-1;drawCard2.commitSpecs(c)}}
                                ClassicButton{text:"Reset";width:58
                                    onClicked:{if(appState){drawCard2.selectedDrawIndex=-1;appState.resetDrawSpecsToDefaults()}}}
            ...
            Item {
                ...
                CompactFrame {
                    ...
                    function rebuildProdMbSorted() {
                        ...
                        // Preserve the backend ordering exactly. ColumnUnitState already
                        // assembles the material-balance rows in physical/product order.
                        // Re-sorting in QML breaks attached stripper ordering because labels
                        // like "Heavy Naphtha Stripper Bottoms" do not carry a [Tray N] suffix.
                        prodMbSorted=rows
                    }
            ...
            Item {
                ...
                function positionAtCurrentMatch() {
                    if (!searchMatches || currentMatchPos < 0 || currentMatchPos >= searchMatches.length)
                        return
                    runLogList2.positionViewAtIndex(searchMatches[currentMatchPos], ListView.Center)
                }
                ...
                function lineMatches(lineText) {
                    ...
                    if (caseSensitive)
                        return line.indexOf(searchText) >= 0
                ...
                CompactFrame {
                    ...
                    Rectangle {
                        id: runLogFindBar
                        anchors { left: parent.left; right: parent.right; top: runLogHdr.bottom; leftMargin: 6; rightMargin: 6; topMargin: 6 }
                        height: 32
                        color: cmdBar
                        border.color: borderIn
                        border.width: 1
                        ...
                        Row {
                            ...
                            CField {
                                ...
                                font.pixelSize: fsLbl
                                text: runLogTab.searchText
                                placeholderText: "RR_SIGN, PH_FLASH_IN, STRIPPER..."
                                onTextChanged: {
                                    runLogTab.searchText = text
                                    runLogTab.refreshSearch()
                                }
                            }
                            ...
                            CheckBox {
                                ...
                                text: "Case"
                                font.pixelSize: fsLbl
                                checked: runLogTab.caseSensitive
                                anchors.verticalCenter: parent.verticalCenter
                                anchors.verticalCenterOffset: 1
                                onToggled: {
                                    runLogTab.caseSensitive = checked
                                    runLogTab.refreshSearch()
                                }
                            }
                            ...
                            Button {
                                ...
                                anchors.verticalCenter: parent.verticalCenter
                                anchors.verticalCenterOffset: 1
                                enabled: runLogTab.searchMatches.length > 0
                                onClicked: runLogTab.previousMatch()
                            }
                            ...
                            Button {
                                ...
                                anchors.verticalCenter: parent.verticalCenter
                                anchors.verticalCenterOffset: 1
                                enabled: runLogTab.searchText !== ""
                                onClicked: {
                                    runLogSearchField.text = ""
                                    runLogTab.clearSearch()
                                }
                            }

                            Text {
                                text: runLogTab.searchText === "" ? "" : (runLogTab.searchMatches.length === 0
                                      ? "0 matches"
                                      : ((runLogTab.currentMatchPos + 1) + " of " + runLogTab.searchMatches.length))
                                font.pixelSize: fsLbl
                                color: textMuted
                                anchors.verticalCenter: parent.verticalCenter
                            }
                    ...
                    ScrollView {
                        anchors{left:parent.left;right:parent.right;top:runLogFindBar.bottom;bottom:parent.bottom;leftMargin:6;rightMargin:6;topMargin:6}
                        clip:true; ScrollBar.vertical.policy:ScrollBar.AsNeeded
                        ListView {
                            id:runLogList2; model:appState?appState.runLogModel:null; spacing:0
                            delegate:Item{
                                width:runLogList2.width;height:rowH
                                property bool rowIsMatch: runLogTab.lineMatches(model.text || "")
                                property bool rowIsCurrent: runLogTab.searchMatches.length > 0
                                                            && runLogTab.currentMatchPos >= 0
                                                            && index === runLogTab.searchMatches[runLogTab.currentMatchPos]
                                Rectangle {
                                    anchors.fill: parent
                                    color: rowIsCurrent ? "#ffe79a" : (rowIsMatch ? "#fff4c7" : (index % 2 === 0 ? rowEven : rowOdd))
                                }
                                Text{
                                    x:4;anchors.verticalCenter:parent.verticalCenter;text:model.text||"";font.pixelSize:fsVal;color:textMain;font.family:"Monospace"
                                }
                                HDivider{anchors.bottom:parent.bottom;width:parent.width;color:"#d0d8e8"}
                            }
                            onCountChanged: {
                                if (runLogTab.searchText !== "")
                                    Qt.callLater(function(){ runLogTab.refreshSearch() })
                                else
                                    Qt.callLater(function(){ runLogList2.positionViewAtEnd() })
                            }
                            Text{anchors.centerIn:parent;visible:!appState||!appState.runLogModel||appState.runLogModel.count===0
            ...
            Item {
                ...
                CompactFrame {
                    ...
                    ListView {
                        ...
                        delegate:Item{
                            width: parent ? parent.width : 0
                            height: Math.max(rowH, diagText.implicitHeight + 8)
                            Rectangle{anchors.fill:parent;color:index%2===0?rowEven:rowOdd}
                            Rectangle{
                                width: 8; height: 8; radius: 2
                                x: 4; y: 6
                                color: model.level==="error" ? errorRed : (model.level==="warn" ? warnAmber : activeBlue)
                            }
                            Text{
                                id: diagText
                                x: 18
