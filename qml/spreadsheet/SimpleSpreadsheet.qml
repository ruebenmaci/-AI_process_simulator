// SimpleSpreadsheet.qml
// Lightweight Excel-style spreadsheet – pure QML, no C++ required.
// Features:
//   • Frozen column/row headers (customisable via colLabels / rowLabels)
//   • Drag-to-resize column widths and row heights (double-click to auto-fit)
//   • Click to select, double-click or F2 to edit inline
//   • Multi-cell selection: click-drag or Shift+Arrow
//   • Ctrl+C copy / Ctrl+V paste – tab/newline format, compatible with Excel
//   • Delete / Backspace clears selected cells
//   • Arrow / Tab / Enter navigation
//   • Basic formula support: =A1+B2, =SUM A1:C5
//   • Public API: setCell(r,c,text), getCell(r,c), clearAll()
...
Item {
    ...
    property int  hdrColW      : 80    // auto-updated by _updateHdrColW() when rowLabels changes
    property int  hdrRowH      : 24

    property color gridColor    : "#b0b0b0"
    property color headerBg     : "#e0e0e0"    // column header + Property corner — medium grey
    property color rowHdrBg     : "#f4f6f8"    // data row headers — matches component list alternating colour
    ...
    // Natural height: header row + all data rows at their default height.
    ...
    // Compute hdrColW from the widest rowLabel string using actual text measurement.
    // Called from measureCanvas.onAvailableChanged so the context is guaranteed ready.
    function _updateHdrColW() {
        ...
        if (rowLabels && rowLabels.length > 0) {
            for (var i = 0; i < rowLabels.length; i++) {
                var lbl = rowLabels[i] ? String(rowLabels[i]) : ""
                var w = Math.ceil(ctx.measureText(lbl).width) + padding
                if (w > best) best = w
            }
        }
        hdrColW = best
    ...
    function setCell(r, c, text) {
        if (!store || store.length === 0) _init()   // auto-init if called before Component.onCompleted
        _set(r, c, text)
    }
    ...
    Item {
        ...
        anchors.fill: parent
            ...
            // Corner — "Property" header, same colour as column headers
            Rectangle {
                x: 0; y: 0; z: 4
                    ...
                    delegate: Item {
                        ...
                        MouseArea {
                            ...
                            onPositionChanged: function(e) {
                                ...
                                var maxW = Math.max(24, root.width - root.hdrColW - otherW - 2)
                                arr[colRH.colIdx] = Math.min(newW, maxW)
                                root.colW = arr
    ...
    // Hidden TextEdit — bridges to the OS system clipboard via .copy() / .paste()
    TextEdit { id: clipBridge; visible: false; width: 0; height: 0 }

    // Hidden canvas used only for text measurement — triggers _updateHdrColW once ready
    Canvas {
        id: measureCanvas
        visible: false; width: 1; height: 1
        onAvailableChanged: if (available) root._updateHdrColW()
    }
